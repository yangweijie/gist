...
<div class="py-6">
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        <!-- 页面标题 -->
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900">创建新 Gist</h1>
            <p class="text-gray-600 mt-2">创建一个新的代码片段</p>
        </div>

        <!-- 创建表单 -->
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <form action="{{ route('gists.store') }}" method="POST" class="p-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                              ...
                              placeholder="简单描述一下这个代码片段...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="filename" name="filename" value="{{ old('filename') }}"
                ...
                <div>
                    ...
                    <textarea id="content" name="content" rows="20" required
                              ...
                              placeholder="在这里输入你的代码...">{{ old('content') }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <x-tag-selector
                        ...
                        placeholder="选择或创建标签..."
                        :max-tags="10" />
                    <p class="mt-1 text-sm text-gray-500">选择现有标签或创建新标签来分类您的 Gist</p>
                    @error('tags')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
